...
import { ErrProductDeleted, ErrProductNotFound } from "../model/error";

export class GetProductByCondService
  implements IQueryHandler<IBaseGetByCond<ProductCondType>, ProductType>
{
  constructor(private readonly _repository: IProductRepository) {}

  query = async (
    command: IBaseGetByCond<ProductCondType>
  ): Promise<ProductType> => {
    ...
    const product = await this._repository.byCond(whereCondition);
    if (!product) {
      throw ErrProductNotFound;
    }

    if (product.status === ProductStatus.Deleted) {
      throw ErrProductDeleted;
    }
