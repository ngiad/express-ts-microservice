...
export const authMiddleware = (
  verifyTokenCommand: ICommandHandler<IVerifyTokenCommand, UserResponseType>
) => {
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (user.status === UserStatus.Deleted)
      throw new ResponseErrorBadRequest("user deleted");
    if (user.status === UserStatus.Banner)
      throw new ResponseErrorBadRequest("user banner");
...
export const userForbiddenMiddleware = () => {
  return (req: Request, res: Response, next: NextFunction) => {
    const user = res.locals.user;
